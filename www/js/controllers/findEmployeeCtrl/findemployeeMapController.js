...
myApp.controller('findemployeeMapController', function (HOSTNAME, $window, $ionicLoading, $scope, $cordovaGeolocation, $ionicPopup, $rootScope, $ionicLoading, $ionicPlatform, $http, $ionicScrollDelegate, $state) {
  $scope.findLocation = {};
  ...
  $scope.searchEmployeeLocation = function () {
    // $ionicLoading.hide();
    $scope.errorMsg = false;
    if ($scope.findLocation.EmployeeKey) {
      console.log($scope.findLocation.EmployeeKey.EmployeeKey);
      ...
      $ionicPlatform.ready(function () {
        ...
        var myLat = "";
        var myLon = "";
        $scope.myPromise = $http.get(HOSTNAME + "/getEmployeesLocation?employeekey=" + $scope.findLocation.EmployeeKey.EmployeeKey + "&OrganizationID=" + $scope.OrganizationID)
          .success(function (response) {
            // $ionicLoading.hide();
